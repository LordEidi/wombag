...
func main() {
	...
	env := util.GetStringFromConfig("env")

	n := negroni.New(negroni.NewRecovery(), negroni.HandlerFunc(wombaglib.OAuthMiddleware), negroni.NewLogger())
	...
	host := util.GetStringFromConfig("www.host")
	port := util.GetStringFromConfig("www.port")
